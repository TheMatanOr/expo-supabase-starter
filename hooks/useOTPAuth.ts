...
/**
 * OTP-based authentication hook for onboarding flow
 *
 * Handles the multi-step OTP authentication process including:
 * - Email verification with OTP codes
 * - User profile creation with onboarding data
 * - Complex error handling for signup/login scenarios
 *
 * Used specifically for the onboarding authentication flow.
 * For general app authentication, use useAuth from @/context/supabase-provider
 */
			...
			try {
				...
				const { data, error } = await supabase
					...
					.upsert({
						...
						gender: onboardingData?.gender?.[0] || null, // Single select
						vision: onboardingData?.vision?.[0] || null, // Single select
						count_per_day: onboardingData?.count_per_day?.[0] || null, // Single select
